// Based heavily on the very excellent https://github.com/minecraft-dotnet/Substrate
    ...
    public class RegionFile : IDisposable
    {
        ...
        public Stream GetChunkDataInputStream(int x, int z)
        {
            ...
            try
            {
                ...
                lock (this.fileLock)
                {
                    if (sectorNumber + numSectors > sectorFree.Count) { return null; }
                    ...
                    else if (version == VERSION_DEFLATE)
                    {
                        ...
                        Stream ret = new ZlibStream(new MemoryStream(data), CompressionMode.Decompress);
                        return ret;
                    }
                    return null;
                }
        ...
        public Stream GetChunkDataOutputStream(int x, int z) => new ZlibStream(new ChunkBuffer(this, x, z), CompressionMode.Compress);

        public Stream GetChunkDataOutputStream(int x, int z, int timestamp) => new ZlibStream(new ChunkBuffer(this, x, z, timestamp), CompressionMode.Compress);
        ...
        class ChunkBuffer : MemoryStream
        {
            ...
            public override void Close()
            {
                if (_timestamp == null)
                {
                    region.WriteChunk(x, z, this.GetBuffer(), (int)this.Length);
                }
                else
                {
                    region.WriteChunk(x, z, this.GetBuffer(), (int)this.Length, (int)_timestamp);
                }
            }
        }

        protected void WriteChunk(int x, int z, byte[] data, int length)
        {
            WriteChunk(x, z, data, length, Timestamp());
        }

        /* write a chunk at (x,z) with length bytes of data to disk */
        protected void WriteChunk(int x, int z, byte[] data, int length, int timestamp)
        {
            if (_disposed)
            {
                throw new ObjectDisposedException("RegionFile", "Attempting to use a RegionFile after it has been disposed.");
            ...
            try
            {
                ...
                if (sectorNumber != 0 && sectorsAllocated == sectorsNeeded)
                {
                    ...
                    WriteChunk(sectorNumber, data, length);
                }
                else
                {
        ...
        private void WriteChunk(int sectorNumber, byte[] data, int length)
        {
            lock (this.fileLock)
            {
                file.Seek(sectorNumber * SectorBytes, SeekOrigin.Begin);
        ...
        public (int x, int y)? ParseCoordinatesFromName()
        {
            int x = 0;
            int z = 0;
